...
@php
    $cfg = catchy_style("offcanvas.directions.{$direction}", match($direction) {
        'left' => [
...
<div
    {{ $attributes->merge([
        'id' => $id,
        'class' => catchy_style('offcanvas.base', 'fixed inset-0 z-50 overflow-hidden'),
        'style' => 'display: none;',
        'role' => 'dialog',
        'aria-modal' => 'true',
...
>
    ...
    <div
        ...
        class="{{ catchy_style('offcanvas.backdrop', 'fixed inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity') }}"
    ...
    >
        <!-- Header -->
        <div class="{{ catchy_style('offcanvas.header', 'flex items-center justify-between border-b border-slate-100 dark:border-slate-800/80 px-6 py-4') }}">
            <h3 id="{{ $id }}-title" class="{{ catchy_style('offcanvas.title', 'text-lg font-semibold text-slate-900 dark:text-slate-100') }}" x-text="title"></h3>
            <button
                type="button"
                @click="close()"
                class="{{ catchy_style('offcanvas.close_btn', 'rounded-lg p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800/60 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500') }}"
            >
        ...
        <!-- Body -->
        <div class="{{ catchy_style('offcanvas.body', 'flex-1 overflow-y-auto px-6 py-4 text-slate-600 dark:text-slate-300') }}">
        ...
        @if(isset($footer))
            <div class="{{ catchy_style('offcanvas.footer', 'border-t border-slate-100 dark:border-slate-800/80 px-6 py-4 bg-slate-50 dark:bg-slate-900/50 flex justify-end gap-3') }}">
                {{ $footer }}
            </div>
        @endif
    </div>
