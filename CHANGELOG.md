...
### Fixed
- Fixed dynamic modal event listeners to support prefixed events (e.g. `catchy:modal-open`).
- Cleaned up unused `hydrateAlpine` utility function.
- Updated documentation and playground examples for Zero Boilerplate vs Custom validation.

## [1.5.2] - 2026-07-06

### Added
- Added custom `<x-catchy-scripts />` Blade component.
- Added `middleware_auto_register` config toggle for opt-out global middleware injection.
- Added premium Inertia-style error overlay iframe modal displaying detailed 500/403/401 server HTML errors in debug mode.
- Added automatic page reload on `419` CSRF token mismatch/expiration.
- Added automatic form validation error highlighting and message injection underneath invalid inputs.

### Changed
- Refactored core PHP classes with PHP 8.1+ constructor property promotion and readonly variables for enhanced strict typing.
- Made the view-transition CSS target ID dynamic, respecting the configured `container_id`.
- Optimized hover prefetching by checking `relatedTarget` boundary to prevent premature cancellation.
